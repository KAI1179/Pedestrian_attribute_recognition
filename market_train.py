...
from multi_task_network_Market import pretrain_model
...
parser.add_argument('--dataset', default='market', type=str, help='dataset:duke')
parser.add_argument('--backbone', default='multi_task_network_Market', type=str, help='backbone: multi_task_network_Market, multi_task_network_Duke')
...
parser.add_argument('--load-path-name', default='net_40.pth', type= str , help='last epoch name')
...
# Model and Optimizer
# ------------------
# model = InceptionV3(num_label, stage= 'train')   #######num_classes=1000, stage='train'
# model = Backbone_nFC(num_label)
# model = resnet50_nfc(num_label, model_name= model_name)
...
ignored_params_10 = list(map(id, model.block_mid_5_1.parameters()))
ignored_params_11 = list(map(id, model.block_mid_5_2.parameters()))
ignored_params_12 = list(map(id, model.block_group_1.parameters()))
ignored_params_13 = list(map(id, model.block_group_2.parameters()))
ignored_params_14 = list(map(id, model.block_group_3.parameters()))
ignored_params_15 = list(map(id, model.block_group_4.parameters()))
ignored_params_16 = list(map(id, model.block_group_5.parameters()))

ignored_params = ignored_params_1 + ignored_params_2 + ignored_params_3 + ignored_params_4 \
                 + ignored_params_5 + ignored_params_6 + ignored_params_7 \
                 + ignored_params_8 + ignored_params_9 + ignored_params_10 + ignored_params_11 + ignored_params_12 \
                 + ignored_params_13 + ignored_params_14 + ignored_params_15 + ignored_params_16
...
optimizer = torch.optim.SGD([
    ...
    {'params': model.block_group_5_add.parameters(), 'lr': 0.1},
    {'params': model.block_group_1_classifier_1.parameters(), 'lr': 0.1},
    {'params': model.block_group_1_classifier_2.parameters(), 'lr': 0.1},
    {'params': model.block_group_2_classifier_1.parameters(), 'lr': 0.1},
    {'params': model.block_group_2_classifier_2.parameters(), 'lr': 0.1},
    {'params': model.block_group_3_classifier.parameters(), 'lr': 0.1},
    {'params': model.block_group_4_classifier.parameters(), 'lr': 0.1},
    {'params': model.block_group_5_classifier.parameters(), 'lr': 0.1},
...
if args.RESUME:
    ...
    checkpoint = torch.load(load_model_dir, map_location='cpu')  # 加载断点
...
def train_model(model, optimizer, scheduler, num_epochs, start_epoch = 1):
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for phase in ['train', 'val']:
            ...
            # epoch_acc_1 = running_corrects_1 / dataset_sizes[phase]
            # epoch_acc_2 = running_corrects_2 / dataset_sizes[phase]

            print('{} Loss: {:.4f} Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc))
            print(lr_scheduler.get_last_lr())  # 打印lr
